FUNCTION /dlwupex/cockpit_rfc_cam_list.
*"----------------------------------------------------------------------
*"*"Interface locale :
*"  IMPORTING
...
*"  CHANGING
*"     VALUE(CT_TAB_WLK1) TYPE  WRF_PRICAT_WLK1_TTY
*"     VALUE(CT_LIST) TYPE  /DLWUPEX/TLIST_TT
*"----------------------------------------------------------------------
  ...
  TRY.
      CALL METHOD (lv_class)=>bapi_create_listing_queue
        EXPORTING
          iv_bu       = iv_bu
          iv_reqid    = iv_reqid
          iv_artkey   = iv_artkey
        CHANGING
          ct_tab_wlk1 = ct_tab_wlk1
          ct_list     = ct_list
          cv_rc       = lv_rc
          ct_msg      = lt_msg.
...
*      ls_msg-matnr      = iv_matnr.
      ...
      ls_msg-idmsg      = '/DLWUPEX/COCKPIT_ART'.
      ls_msg-num        = '001'.
      ls_msg-message_v1 = 'BAPI_CREATE_LIST_QUEUE'.
